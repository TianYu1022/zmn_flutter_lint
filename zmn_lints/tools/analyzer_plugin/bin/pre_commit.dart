...
Future<void> main(List<String> args) async {
  final String workingDirectory = args.isNotEmpty ? args.first : Directory.current.path;
  ...
  final List<String> untracked = CandiesAnalyzerPlugin.processRun(
    ...
    throwException: false,
    workingDirectory: workingDirectory,
  ).trim().split('\n').map((String e) => join(workingDirectory, e).trim()).toList();

  final List<String> analyzeFiles = <String>[...diff, ...untracked].where((String element) => element.startsWith(workingDirectory)).toList();
...
void _printErrors(List<String> errors, int inMilliseconds) {
  ...
  if (errors.isEmpty) {
    ...
    print('');
    print(errors.map((String e) => '  ${e.getHighlightErrorInfo()}').join('\n\n'));
    print('${'\n${errors.length} issues found.'.wrapAnsiCode(foregroundColor: AnsiCodeForegroundColor.red)}  ${seconds}s');
    print('Please fix the errors and then submit the code.'.wrapAnsiCode(foregroundColor: AnsiCodeForegroundColor.red));
  }
}
