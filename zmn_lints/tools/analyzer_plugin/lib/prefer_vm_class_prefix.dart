...
class PerferVmClassPrefix extends DartLint {
  @override
  String get code => 'perfer_zmn_viewmodel_class_prefix';
  ...
  SyntacticEntity? matchLint(AstNode node) {
    if (node is ClassDeclaration) {
      ...
      final String nodeClassName = name.substring(startIndex);
      if (nodeClassName.contains('ViewModel') && !nodeClassName.startsWith('ZMN')) {
        return node.name2;
      }
    }
    return null;
  ...
  @override
  String get message => 'Define a ViewModel class name start with ZMN';
  ...
  Future<List<SourceChange>> getDartFixes(
    ...
    final Iterable<DartAnalysisError> cacheErrors = config.getCacheErrors(resolvedUnitResult.path, code: code).whereType<DartAnalysisError>();

    final Map<DartAnalysisError, Set<SyntacticEntity>> references = _findClassReferences(cacheErrors, resolvedUnitResult);

    return <SourceChange>[
      await getDartFix(
        resolvedUnitResult: resolvedUnitResult,
        message: 'Use ZMN as a viewmodel class prefix.',
      ...
      if (cacheErrors.length > 1)
        await getDartFix(
          resolvedUnitResult: resolvedUnitResult,
          message: 'Use ZMN as a viewmodel class prefix where possible in file.',
          buildDartFileEdit: (DartFileEditBuilder dartFileEditBuilder) {
  ...
  void _fix(
    ...
    final String replace = '${nameString.substring(0, startIndex)}ZMN${nameString.substring(startIndex)}';

    for (final SyntacticEntity match in references) {
      dartFileEditBuilder.addSimpleReplacement(SourceRange(match.offset, match.length), replace);
    }
  }
  ...
  Map<DartAnalysisError, Set<SyntacticEntity>> _findClassReferences(
    ...
    final Map<DartAnalysisError, Set<SyntacticEntity>> references = <DartAnalysisError, Set<SyntacticEntity>>{};
    final Map<String, DartAnalysisError> classNames = <String, DartAnalysisError>{};
    ...
    resolvedUnitResult.unit.accept(_FindClassReferenceVisitor(references, classNames));
