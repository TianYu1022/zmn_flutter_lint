...
class ZmnLintMustCallSuperDispose extends DartLint with CallSuperDisposeMixin {
  @override
  ...
  Future<List<SourceChange>> getDartFixes(
    ...
    final Iterable<DartAnalysisError> cacheErrors = config.getCacheErrors(resolvedUnitResult.path, code: code).whereType<DartAnalysisError>().where((DartAnalysisError element) {
      final AstNode astNode = element.astNode;
      return _hasFix(astNode) && _matchLint(astNode as MethodDeclarationImpl) == astNode;
    });
...
class ZmnLintEndCallSuperDispose extends DartLint with CallSuperDisposeMixin {
  @override
  ...
  Future<List<SourceChange>> getDartFixes(
    ...
    final Iterable<DartAnalysisError> cacheErrors = config.getCacheErrors(resolvedUnitResult.path, code: code).whereType<DartAnalysisError>().where((DartAnalysisError element) {
      final AstNode astNode = element.astNode;
      return _hasFix(astNode) && _matchLint(astNode as MethodDeclarationImpl) is ExpressionStatementImpl;
    });
    ...
    return <SourceChange>[
      ...
      if (cacheErrors.length > 1)
        await getDartFix(
          resolvedUnitResult: resolvedUnitResult,
          message: 'call super.dispose at the end of this method where possible in file.',
          buildDartFileEdit: (DartFileEditBuilder dartFileEditBuilder) {
            for (final DartAnalysisError error in cacheErrors) {
              _fix1(
                dartFileEditBuilder,
                _matchLint(error.astNode as MethodDeclarationImpl) as ExpressionStatementImpl,
                error.astNode as MethodDeclarationImpl,
              );
            }
          },
  ...
  SyntacticEntity? matchLint(AstNode node) {
    if (node is MethodDeclarationImpl) {
      if (node.name2.toString() == 'dispose') {
        ...
        // remove the metadata
        if (result != null && result is MethodDeclarationImpl && result.metadata.isNotEmpty) {
    ...
    for (final ChildEntity element in node.body.namedChildEntities) {
      if (element.value is BlockImpl) {
        ...
        for (final Statement statement in statements) {
          if (statement is ExpressionStatementImpl && statement.expression.toString() == 'super.dispose()') {
...
void _fix1(DartFileEditBuilder dartFileEditBuilder, ExpressionStatementImpl result, MethodDeclarationImpl astNode) {
...
void _fix(DartFileEditBuilder dartFileEditBuilder, MethodDeclarationImpl astNode) {
  dartFileEditBuilder.addSimpleInsertion(astNode.end - 1, 'super.dispose();');
...
bool _hasFix(AstNode astNode) {
  return astNode is MethodDeclarationImpl && astNode.body is BlockFunctionBodyImpl;
}
