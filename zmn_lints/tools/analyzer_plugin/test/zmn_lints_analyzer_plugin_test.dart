...
import 'dart:io';
import 'package:zmn_lints_analyzer_plugin/zmn_lints_analyzer_plugin.dart';
...
void main() {
  ...
  final String debugFilePath = path.join(path.current, 'example', 'lib', 'main.dart');
  test('getErrors', () async {
    final String debugFilePath = Directory.current.parent.parent.parent.path;
    final AnalysisContextCollection collection = AnalysisContextCollection(includedPaths: <String>[debugFilePath]);
    ...
    for (final AnalysisContext context in collection.contexts) {
      for (final String file in context.contextRoot.analyzedFiles()) {
        ...
        for (final AnalysisError error in errors) {
          final List<AnalysisErrorFixes> fixes = await myPlugin.getAnalysisErrorFixesForDebug(EditGetFixesParams(file, error.location.offset), context).toList();
          print(fixes.length);
        }
  ...
  test('ignore for this line', () async {
    final ResolvedUnitResult result = await resolveFile2(path: debugFilePath) as ResolvedUnitResult;
    ...
    final int offset = result.content.indexOf('onDoubleTap: () => setState(() {})');
    final CharacterLocation characterLocation = lineInfo.getLocation(offset);
    final CandiesAnalyzerPluginIgnoreInfo ignore = CandiesAnalyzerPluginIgnoreInfo.forDart(result);
  ...
  test('AnalysisContextCollection', () async {
    // final String theAnalyzePath = path.join(path.current, 'example');
    final String theAnalyzePath = Directory.current.parent.parent.parent.path;
    final AnalysisContextCollection collection = AnalysisContextCollection(includedPaths: <String>[theAnalyzePath]);
    for (final AnalysisContext context in collection.contexts) {
      ...
      if (optionsFilePath != null) {
        ...
        if (yaml.nodes['zmn_lints'] is YamlMap) {
          final YamlMap pluginConfig = yaml.nodes['zmn_lints'] as YamlMap;
          if (pluginConfig.nodes['include'] is YamlList) {
            final YamlList includePatterns = pluginConfig.nodes['include'] as YamlList;

            final List<Glob> includePatterns2 = includePatterns.map((dynamic e) => Glob(path.separator, e.toString())).toList();

            bool include(String path) {
              if (includePatterns2.isEmpty) {
                return true;
              }

              for (final Glob includePattern in includePatterns2) {
            ...
            final String testFile = path.join(path.current, 'example', 'lib', 'include', 'test.dart');

            final String relative = path.relative(testFile, from: context.contextRoot.root.path);
